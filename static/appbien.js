...
function ouvrirModifEtudiant(id_etudiant, ine, nom, prenom, mail, filiere, semestre) {
    ...
    // Ouvrir sans réinitialiser (on a déjà rempli)
    $('overlay')?.classList.remove('hidden');
...
async function loadEtudiants() {
    ...
    tb.innerHTML = studentsData.map(e => `<tr>
        ...
        <td>${e.filiere || e.filière || '—'}</td>
        <td>${e.semestre || '—'}</td>
        <td>${e.mail}</td>
        <td class="ac">
            <button class="btn-icon" title="Modifier"
                onclick="ouvrirModifEtudiant(${e.id_etudiant},'${esc(e.ine)}','${esc(e.nom)}','${esc(e.prenom)}','${esc(e.mail)}','${esc(e.filiere||e.filière||'')}',${e.semestre||1})">✏️</button>
...
async function loadEvalsResp() {
    ...
    const rows = await Promise.all(stages.map(async st => {
        ...
        let ev = null;
        if (ine && rap) {
            const rEv = await api(`/responsable/evaluation/${ine}`);
            if (rEv.ok && rEv.data?.id_rapport) ev = rEv.data;
        }
...
function ouvrirEvalResp(ine, id_rapport, chemin, titre) {
    _evalCtx = { role: 'responsable', ine, id_rapport, chemin, titre };
...
async function loadMonStage() {
    ...
    c.innerHTML = `
        <div class="info-card">
            ...
            <div class="info-row"><span class="lbl">Période</span><span class="val">${st.date_debut || '—'} → ${st.date_fin || '—'}</span></div>
...
async function loadMonRapport() {
    ...
    // Récupérer id_stage si pas en mémoire
    if (!currentUser._id_stage) {
        const rs = await api(`/etudiant/stage/${currentUser.id_etudiant}`);
        if (rs.data?.id_stage) {
    ...
    if (!currentUser._id_stage) {
        c.innerHTML = '<div class="info-placeholder">Aucun stage — rapport non disponible.</div>';
...
async function submitRapport() {
    clearErr('err-m-rapport');
    const id_stage       = currentUser._id_stage;
    const titre          = $('rp-titre')?.value.trim();
    const chemin_fichier = $('rp-chemin')?.value.trim();
    const contenu        = $('rp-contenu')?.value.trim() || null;
    const fileInput      = $('rp-fichier');   // input type="file" optionnel si présent dans le HTML

    if (!titre) { setErr('err-m-rapport', 'Le titre est requis.'); return; }
    if (!chemin_fichier && (!fileInput || !fileInput.files?.length)) {
        setErr('err-m-rapport', 'Fournissez un chemin ou sélectionnez un fichier.'); return;
    }
    ...
    let r;
    // Si un fichier est sélectionné → multipart/form-data
    if (fileInput && fileInput.files?.length) {
        const fd = new FormData();
        fd.append('id_stage', id_stage);
        fd.append('titre', titre);
        if (contenu) fd.append('contenu', contenu);
        fd.append('rapport', fileInput.files[0]);
        r = await apiUpload(existe ? '/rapport/modifier' : '/rapport/ajouter', fd);
    } else {
        // Sinon JSON avec chemin textuel
        const payload = { id_stage, titre, chemin_fichier, contenu };
        r = await api(existe ? '/rapport/modifier' : '/rapport/ajouter', 'POST', payload);
    }
    ...
    } else { setErr('err-m-rapport', r.data.message || 'Erreur.'); }
}
...
async function loadSupStages() {
    ...
    const rows = await Promise.all(stages.map(async st => {
        ...
        // Récupérer INE de l'étudiant
        const rEt = await api(`/etudiants/${st.id_etudiant || st.etudiant}/by-id`).catch(() => ({ data: null }));
        const ine = rEt.data?.ine || null;

        // Note existante du superviseur
        let noteExist = null;
        if (ine) {
            const rEv = await api(`/superviseur/evaluation/${ine}`);
        ...
        return `<tr>
            <td>${ine ? `<code>${ine}</code>` : '#'+st.etudiant}</td>
            <td>${st.titre}</td>
            ...
            <td class="ac">
                ${btn}
                ${noteExist != null ? `<span style="color:var(--ok,green);margin-left:.5rem">✔ ${noteExist}/20</span>` : ''}
            </td></tr>`;
    }));
...
async function loadRappStages() {
    ...
    const rows = await Promise.all(stages.map(async st => {
        ...
        let noteExist = null;
        if (ine) {
            const rEv = await api(`/rapporteur/evaluation/${ine}`);
...
function ouvrirEvalEncadreur(role, id_rapport, chemin, titre, ine) {
    _evalCtx = { role, id_rapport, chemin, titre, ine };
...
async function confirmDelete() {
    ...
    if (type === 'etudiant')
        r = await api(`/etudiants/${id}`, 'POST', { id_responsable: respId(), id_etudiant:id });
    else if (type === 'superviseur')
        r = await api(`/superviseurs/supprimer`, 'POST', { id_responsable: respId(),id_superviseur:id });
    else if (type === 'rapporteur')
        r = await api(`/rapporteurs/supprimer`, 'POST', { id_responsable: respId(), id_rapporteur:id});
    else if (type === 'stage')
        r = await api(`/stages/supprimer`, 'POST', { id_responsable: respId(),id_stage:id});
